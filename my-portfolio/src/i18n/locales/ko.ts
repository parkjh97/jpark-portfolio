const ko = {
  ...
  masthead: {
    ...
    summary: [
      "복잡한 문제를 풀어내는 과정에서 사용자를 가장 먼저 떠올리는 개발자입니다.",
      "기술과 스토리를 엮어 팀이 나아갈 방향을 함께 설계합니다.",
    ],
    overviewTitle: "안녕하세요, 박정현입니다.",
    overviewBody:
      "기획 테이블에서 서비스의 첫 줄을 맞추고, 배포 이후에는 사용자의 목소리로 다시 문장을 다듬습니다. 프론트엔드와 백엔드를 모두 이해하는 시야로 문제의 본질을 짚어내고, 사용자 경험이 자연스럽게 이어지도록 환경을 설계합니다. 팀이 안심하고 개발할 수 있도록 문서화와 자동화를 습관화하고 있으며, 작은 습관이 곧 조직의 속도와 품질을 지켜 준다고 믿습니다.",
    narrativeTitle: "제가 일하는 방식",
    narrative: [
      "첫 팀 서비스였던 대학 시절 프로젝트를 시작으로, 고객이 직접 사용하는 화면이 어떻게 살아 움직이는지 지켜보며 개발자의 길을 다졌습니다. 이후 커머스, 커뮤니티, 사내 플랫폼 등 다양한 도메인을 경험하며 '기술적인 완성도'와 '사용자 경험'을 동시에 관리하는 방법을 배웠습니다. 기능을 출시하는 것에 그치지 않고, 서비스가 지향하는 가치가 고객에게 잘 전달되고 있는지 꾸준히 검증하는 것을 무엇보다 중요하게 생각합니다.",
      "당근마켓에서는 비즈프로필 기반 기능을 통해 동네 사장님과 고객이 만나는 경험을 설계했습니다. 지도 경험의 유입 전략을 세우고, 웹뷰 플랫폼을 개편하여 성능과 운영 효율을 끌어올렸습니다. 이 과정에서 프론트엔드 모듈화, API 설계, 데이터 파이프라인, 모니터링 체계 등을 직접 구축하며 팀 전체가 신뢰할 수 있는 개발 환경을 마련했습니다.",
