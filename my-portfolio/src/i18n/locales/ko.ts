const ko = {
  ...
  masthead: {
    ...
    competenciesHeading: "핵심 역량",
    competencies: [
      { title: "문제 중심 개발", desc: "요구사항 분석을 기반으로 문제의 본질을 구조적으로 해결하는 개발 역량" },
      { title: "협업 중심 커뮤니케이션", desc: "기획·디자인 요구사항을 개발 구조로 연결하고 구현 방향을 정렬하는 협업 역량" },
      { title: "풀스택 개발 경험", desc: "Spring API 설계부터 React/Next.js UI 구현까지 서비스 전반 구현 역량" },
      { title: "대규모 데이터 시각화 경험", desc: "GIS·교통 빅데이터를 지도·차트 기반 인터랙티브 대시보드로 구현한 경험" },
    ],
    detailHeading: "세부 소개",
    detail: [
      ...
      "기술은 그 과정에서 선택되는 도구일 뿐이며, 저는 항상 팀과 사용자 사이의 공통 언어를 만들기 위해 노력해왔습니다.",
      "단단한 설계와 책임 있는 구현, 그리고 협업을 통해 결과를 만듭니다."
    ],
    cta: "더 알아보기 ↓",
  },
  about: {
  ...
  projects: {
    ...
    readmes: {
      erp: {
        ...
        details: [
          {
            title: "Next.js App Router, TypeScript 기반으로 화면과 API를 함께 구성",
            background: "Spring/Java 중심의 팀 환경에서 풀스택 개발을 주도해야 하는 상황이었고, 별도 백엔드 서버 없이 화면과 API 로직을 하나의 코드베이스에서 관리할 수 있는 구조가 필요했습니다.",
            result: "프론트엔드와 백엔드가 분리되지 않은 빠른 개발 사이클로 기능 구현과 수정 대응 속도를 높일 수 있었습니다.",
          },
          {
            title: "약 80개의 API와 30개 이상의 페이지로 구성된 사내 업무 서비스에서 공통 컴포넌트, fetch 유틸, 상태 훅으로 반복 기능을 분리",
            background: "페이지와 기능이 빠르게 늘어나는 구조에서, 유사한 UI 패턴과 데이터 요청 로직이 반복적으로 구현되며 코드 중복과 유지보수 비용이 증가하는 문제가 있었습니다.",
            result: "공통 컴포넌트, fetch 유틸, 상태 훅으로 반복 기능을 분리하여 코드 재사용성을 높이고, 기능 확장 시에도 개발 생산성과 유지보수성을 안정적으로 유지할 수 있었습니다.",
          },
          {
        ...
        reflection: [
          ...
          "Next.js를 처음 접한 상태에서 공식 문서와 소스 코드를 기반으로 빠르게 학습하고, 팀 내 도입을 단독으로 주도했습니다. 전 과정을 책임지는 환경에서 기술 선택의 근거를 스스로 판단하며, 설계에 대해 더 깊이 고민하는 습관을 갖게 되었습니다."
        ]
      },
      si: {
        overview: {
          description: "행정 담당자가 대중교통 운영 현황과 이용 패턴을 지도와 차트로 분석할 수 있도록 만든 웹 서비스입니다. 버스 노선, 정류장, 지역별 수요, 수익성, 운영 정보 같은 데이터를 한 화면에서 검색하고 비교할 수 있도록 구성했습니다.",
          team: "웹 개발자(3명) + 디자이너(1명) + 데이터 엔지니어(1명) 협업",
          role: "풀스택 개발 — 주요 기능 5개의 Spring 기반 API 개발, 공간 데이터 쿼리 작성 및 프론트엔드 구현 담당",
        },
        details: [
          {
            title: "대규모 지리 데이터를 줌·패닝·선택 상태에 따라 필요한 지도 레이어만 갱신하는 구조 구현",
            background: "지도 기반 분석 화면에서 수천 개 이상의 데이터를 한 번에 렌더링할 경우 브라우저 성능 저하와 UI 반응 지연 문제가 발생할 수 있었습니다.",
            result: "화면 상태에 따라 필요한 레이어만 갱신하도록 구조를 설계해 렌더링 부담을 줄이고, 대량의 지도 데이터를 포함한 분석 화면에서도 교통 데이터를 직관적으로 탐색할 수 있는 지도 UX를 구현했습니다."
          },
          {
            title: "비동기 이벤트 충돌을 제어하기 위한 커스텀 Scheduler 기반 실행 큐 설계",
            background: "지도와 UI에서 발생하는 비동기 이벤트가 중첩되며 실행 순서가 보장되지 않아, 데이터와 UI 상태 불일치 및 예측 불가능한 동작이 발생하는 문제가 있었습니다.",
            result: "Generator와 큐 기반 Scheduler 모듈을 직접 구현하여 이벤트 실행 순서를 제어하고, 상태 흐름을 일관되게 유지하는 구조를 구축했습니다. 이를 통해 복잡한 상호작용 환경에서도 안정적인 UI 동작을 확보했습니다."
          },
          {
        ...
        reflection: [
          "SI 프로젝트 특성상 고객 요구사항이 빈번하게 변경되는 환경에서, 초기 설계 단계부터 확장성과 변경 대응을 고려하는 것이 개발 효율에 큰 영향을 준다는 것을 경험했습니다.",
          "지도 기반 분석 서비스에서 복잡한 UI 이벤트와 비동기 흐름을 다루며, 단순 구현이 아닌 실행 순서와 상태 흐름을 제어하는 구조 설계의 중요성을 체감했습니다.",
        ],
      },
      viewT: {
        overview: {
          description: "내비게이션 데이터, 통신 데이터 등 다양한 교통 빅데이터를 기반으로 도로 구간별 통행량·속도·혼잡도 등의 지표를 산출하고, 이를 지도 위에서 시각화하여 교통 흐름을 분석할 수 있는 교통 데이터 분석 플랫폼입니다.",
      ...
      hs: {
        ...
        details: [
          ...
          },
          {
            ...
            background: "대규모 GIS 데이터 시각화와 사용자 인터랙션을 함께 처리해야 하는 상황에서, 하나의 방식으로는 성능과 데이터 활용을 동시에 만족시키기 어려웠습니다.",
            result: "단순 시각화는 WMS 기반으로 처리해 렌더링 성능을 확보하고, 클라이언트에서 데이터 가공 및 인터랙션이 필요한 경우에는 WFS를 활용하는 구조로 설계하여 성능과 유연성을 동시에 확보했습니다."
          }
        ],
        reflection: [
          "기존에 장기간 운영되던 API 아키텍처를 분석하고 그 구조 안에서 신규 기능을 확장하는 경험을 통해, 단순 기능 구현을 넘어 기존 시스템과의 정합성을 고려한 개발 역량을 기를 수 있었습니다.",
          "교차로 단위의 복합 데이터를 안정적으로 조회하기 위한 SQL 구조를 설계하면서, 다중 조인 환경에서도 성능을 유지할 수 있는 데이터 조회 구조 설계 경험을 확보했습니다.",
          "GeoServer WMS와 WFS를 목적에 따라 분리 적용하는 과정에서, 대규모 GIS 데이터를 효율적으로 시각화하기 위한 지도 렌더링 전략을 설계하는 경험을 할 수 있었습니다."
        ],
      },
      viewTexportX: {
        overview: {
          ...
          team: "웹 개발자 (2명)",
          role: "프론트엔드 개발 — 아키텍처 설계부터 구현까지 전 과정 담당",
        },
        details: [
          {
